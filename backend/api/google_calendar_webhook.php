...
$row = $conn->query("SELECT userId, syncToken FROM calendar_channels WHERE channelId='" . $conn->real_escape_string($channelId) . "'")->fetch_assoc();
if (!$row) { http_response_code(200); exit; }
$userId = $row['userId'];
...
$query = $row['syncToken'] ? ['syncToken'=>$row['syncToken']] : ['updatedMin'=>gmdate('Y-m-d\TH:i:s\Z', time()-3600)];
...
if (isset($res['items'])) {
  foreach ($res['items'] as $ev) {
    ...
    if ($link && $status !== 'cancelled') {
      $aid = $link['crmAppointmentId'];
      $stmt = $conn->prepare("UPDATE appointments SET service = ?, date = ?, time = ?, updatedAt = NOW() WHERE id = ?");
      $time = substr($start, 11, 5);
      $date = substr($start, 0, 10);
      $stmt->bind_param('ssss', $summary, $date, $time, $aid);
      $stmt->execute(); $stmt->close();
    } elseif ($link && $status === 'cancelled') {
      $aid = $link['crmAppointmentId'];
      $stmt = $conn->prepare("UPDATE appointments SET status='cancelado', updatedAt = NOW() WHERE id = ?");
      ...
      $stmt->bind_param('sssssss', $id, $summary, $date, $time, $createdAt, $updatedAt);
      $stmt->execute(); $stmt->close();
      ...
      $stmt->bind_param('sss', $userId, $id, $gid);
      $stmt->execute(); $stmt->close();
    }
  }
}
if (isset($res['nextSyncToken'])) {
  ...
  $stmt->bind_param('ss', $res['nextSyncToken'], $channelId);
  $stmt->execute(); $stmt->close();
}
